...
final class EuclideanDistanceTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        self.textureAddConstantFloat = try .init(
            context: self.context,
            scalarType: .float
        )
        let sourceImageFileURL = Bundle.metalComputeToolsTestsResources.url(
            forResource: "Shared/255x121",
            withExtension: "png"
        )!
        self.source = try self.context.texture(
            from: .initFromURL(sourceImageFileURL),
    ...
    func testEuclideanDistance() throws {
        ...
        XCTAssertLessThanOrEqual(diff, 4)
    }
}
