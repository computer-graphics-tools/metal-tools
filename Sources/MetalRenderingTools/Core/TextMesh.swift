...
final class TextMesh {
    ...
    init(
        ...
        let fontSize = UIFont.calculateFontSizeToFit(
            rect: rect,
        ...
        self.enumerateGlyphs(in: frame) { glyph, glyphIndex, glyphBounds in
            ...
            vertices[v] = .init(
                position: .init(minX, maxY, 0, 1),
                texCoords: .init(minS, maxT)
            )
            v += 1
            vertices[v] = .init(
                position: .init(minX, minY, 0, 1),
                texCoords: .init(minS, minT)
            )
            v += 1
            vertices[v] = .init(
                position: .init(maxX, minY, 0, 1),
                texCoords: .init(maxS, minT)
            )
            v += 1
            vertices[v] = .init(
                position: .init(maxX, maxY, 0, 1),
        ...
        self.vertexBuffer = try device.buffer(
            with: vertices,
            options: .storageModeShared
        )
        self.indexBuffer = try device.buffer(
            with: indices,
            options: .storageModeShared
        )
    }
